...
- The **Finding Outcome Distribution** pie counts every assessment check once. Insufficient-data slices include the attributed endpoint type, so Microsoft Graph permission failures are visibly distinct from base, parameterised, mixed and unattributed sources. A single adjacent Sankey shows how endpoint source types flow into the same finding outcomes. Exact manifest endpoint-execution outcomes remain available in the collection-type breakdown and request-health tables.
