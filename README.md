...
- Each collection also creates `azure_public_endpoint_topology_<timestamp>.json`. This normalises customer-owned Public IP resources and prefixes, VM and VMSS addresses, Load Balancer and Application Gateway frontends, NAT Gateway, Bastion, Azure Firewall and virtual-network-gateway attachments, and service-managed addresses or names exposed by API Management, App Service and slots, Functions and slots, App Service Environments, Container Instances, Container Apps, AKS, Front Door/CDN, Traffic Manager, Storage, Key Vault, Container Registry, databases, messaging, IoT and analytics services. Records distinguish inbound, outbound and management use; retain owner, attachment, configured downstream targets, ports, confidence and source endpoint identifiers; and do not expand a public prefix into fabricated individual allocations.
- `azure_public_endpoint_topology_coverage_<timestamp>.json` records collection completeness for every topology input. Unauthorised, failed, skipped and uncollected sources remain visible instead of being treated as an empty inventory. The associated finding only treats the Public IP inventory as complete when its registered collection endpoint supports that conclusion.
