...
- Service FQDNs are retained as configuration evidence but are not resolved automatically. Automatic lookup would disclose tenant-derived hostnames to the workstation's configured DNS resolver, and a shared or changing Microsoft service address would not prove exclusive tenant ownership. AKS Services and Ingress objects also remain outside this ARM-only topology because reading them requires separate Kubernetes data-plane authorisation. Shared Microsoft runtime infrastructure and arbitrary runtime destinations cannot be represented as tenant-owned components.
