...
Graph workload findings use the versioned Microsoft public-guidance baseline in `azure_assess/graph_guidance.py`. They cover active identity risk and recommendations, synchronisation and provisioning failures, privileged-user authentication strength, entitlement and permanent directory/group/Azure-resource PIM assignments, PIM alerts, anonymous Microsoft 365 sharing, stale or non-compliant Intune devices, missing Intune controls, assigned wildcard custom Intune roles, BitLocker metadata coverage, applicable Global Secure Access controls, Defender for Identity health, significant unresolved incidents and positive built-in hunting results.
