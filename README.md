...
- Every registered endpoint also has exactly one assessment role. **Automated** endpoints feed one or more deterministic finding checks. **Supporting** endpoints supply scope, parameters or enrichment to those checks but cannot independently establish a pass or failure. **Manual** endpoints return security-relevant context for which there is no conservative, tenant-independent pass/fail rule. The reverse-coverage audit fails when a registered automated endpoint is not referenced by a current check or when a manual/supporting declaration is stale.
...
- The findings view in `azure-present.py` provides a **Validated** checkbox for every raised finding and every item in `manual_assessment_required`. Enter the analyst's name before changing a checkbox; the UI records a `confirmed` disposition and a server-generated UTC review time in `<input-dir>/azure-findings-review.json`. Clearing a raised finding records a reviewed `candidate` disposition; clearing a manual item returns it to `inconclusive`. Existing analyst confidence, contextual severity and notes are preserved.
...
- Manual/context endpoints create one stable **Manual Assessment Required** item per endpoint definition, regardless of fan-out execution count, and are available through that status in the Findings viewer. Its title is the friendly endpoint name, its reason is intentionally blank, and its displayed count is the number of records in retained, viewable source datasets. Failed, unauthorised, missing, non-viewable or empty manual sources remain `no_data_to_assess` and do not appear under the manual status. Supporting-only executions never create manual findings. Manual items remain absent from ordinary SARIF but can be analyst-confirmed and included in the validated SARIF export.
