...
- Manual/context endpoints create one stable **Manual Assessment Required** item per endpoint definition, regardless of fan-out execution count, and are available through that status in the Findings viewer. Failed, unauthorised or empty manual sources remain `no_data_to_assess`; only retained usable evidence can request analyst assessment. Supporting-only executions never create manual findings. Manual items are informational review records and are excluded from SARIF, which continues to contain only findings in the `found` state.
