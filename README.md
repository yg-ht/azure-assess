...
- Collection phase messages are flushed immediately. Microsoft Graph progress identifies the current endpoint number, friendly endpoint name, workload, API channel and completion totals. Long pagination, fan-out, throttling and Microsoft 365 audit-query polling emit bounded heartbeat messages. These messages intentionally omit access tokens, query IDs, parent object IDs and request URLs. The diagnostic Azure CLI version lookup has a 30-second timeout and cannot block collection indefinitely.
...
- Each finding includes a versioned `reporting` object containing deduplicated `assets`, content-addressed `observations`, and `provenance`. Existing `evidence` and `references` structures remain compatible; endpoint attribution fields are additive.
- Every current check declares its expected collection source in `references.endpoint_source_type`, and the same value is copied to `reporting.provenance.endpoint_source_type`. `Base` covers Azure base and parameterised endpoints, `Graph` covers the native Microsoft Graph runner, `Either` means either plane can independently supply the evidence, and `BaseAndGraph` means the check correlates evidence that requires both planes. `reporting.provenance.declared_endpoint_ids` and `declared_endpoint_patterns` preserve the declaration even when no matching execution was recorded. Stable endpoint IDs are used by the newer evaluators; retained endpoint patterns keep historical datasets readable. The automated endpoint-attribution audit fails if a check has no declaration, no current registry match, or an invalid source type.
- Every registered endpoint also has exactly one assessment role. **Automated** endpoints feed one or more deterministic finding checks. **Supporting** endpoints supply scope, parameters or enrichment to those checks but cannot independently establish a pass or failure. **Context** endpoints remain available for inventory and investigation but create neither a finding nor an analyst task. **Manual** endpoints are a curated subset for which the collected records expose a concrete security decision that genuinely requires tenant-specific knowledge. Each manual definition records its review question, applicability, relevant evidence fields, rationale and possible outcomes. The reverse-coverage audit fails when a registered automated endpoint is not referenced by a current check or when a manual, context or supporting declaration is stale.
- The guest MFA-capability check considers enabled guest accounts only and uses complete Microsoft Graph user-registration evidence before treating a missing registration record as an observation. Its title describes the locally reported `isMfaCapable` signal: it does not claim that a guest is unauthenticated or prove that Conditional Access and trusted home-tenant MFA are ineffective. The legacy `entra_unauthenticated_guest_users_present` finding ID remains stable for existing consumers.
...
- Each collection also creates `azure_public_endpoint_topology_<timestamp>.json`. This normalises customer-owned Public IP resources and prefixes, public DNS address and alias records, VM and VMSS addresses, Load Balancer and Application Gateway frontends, NAT Gateway, Bastion, Azure Firewall and virtual-network-gateway attachments, and service-managed addresses or names exposed by API Management, App Service and slots, Functions and slots, App Service Environments, Container Instances, Container Apps, AKS, Front Door/CDN, Traffic Manager, Storage, Key Vault, Container Registry, databases, messaging (including Relay and SignalR), IoT, HDInsight and other tenant-specific analytics services. Route records preserve the applicable rule, frontend, listener or path map, ports, protocols, enabled state, backend pool members and configured origin targets. Exact configured control-plane address or hostname matches are retained separately as candidate component associations; DNS co-resolution alone does not create a relationship because a shared platform address does not prove exclusive ownership. Public prefixes are not expanded into fabricated individual allocations.
- `azure_public_endpoint_topology_coverage_<timestamp>.json` records collection completeness for every topology input. Unauthorised, failed, skipped and uncollected sources remain visible instead of being treated as an empty inventory. DNS coverage separately records attempted hostnames, successful public resolutions, answers without a public address and failures. The associated finding only treats the Public IP inventory as complete when its registered collection endpoint supports that conclusion.
- Service FQDNs are resolved through the collector host's configured DNS resolver. This discloses the collected tenant-derived hostnames to that resolver; run the collector only where that disclosure is authorised. Returned A and AAAA addresses are labelled as point-in-time DNS observations and never treated as proof of exclusive tenant ownership. AKS Services and Ingress objects remain outside this ARM-only topology because reading them requires separate Kubernetes data-plane authorisation. Shared Microsoft runtime infrastructure and arbitrary runtime destinations cannot be represented as tenant-owned components.
...
- Thirty-day Graph activity is assessed for successful legacy-authentication sign-ins, security-sensitive directory changes, Exchange forwarding/delegation changes and anonymous or explicitly external SharePoint sharing. Intune findings inspect legacy device configurations and the per-policy Settings Catalog settings fan-out, while Intune and Global Secure Access inventories add direct disabled/unhealthy control findings. Permanent active and eligible PIM schedules are assessed across directory, group and Azure-resource roles; assignment and eligibility requests seeking permanent access are assessed together; explicitly disabled PIM alert configurations are raised. Function Apps with a wildcard CORS origin, Batch accounts with public or shared-key access, and Data Factory instances with public access are also raised from their collected configuration.
...
- The findings view in `azure-present.py` provides a **Validated** checkbox for every raised finding and every item in `manual_assessment_required`. Manual items show the curated review question, applicability, evidence fields, rationale and possible outcomes; an endpoint is not added to this list merely because no automated rule exists. Enter the analyst's name before changing a checkbox; the UI records a `confirmed` disposition and a server-generated UTC review time in `<input-dir>/azure-findings-review.json`. Clearing a raised finding records a reviewed `candidate` disposition; clearing a manual item returns it to `inconclusive`. Existing analyst confidence, contextual severity and notes are preserved. Review-file entries for retired catch-all manual IDs are retained but ignored, so an earlier review file does not prevent the refined assessment from loading.
- Curated manual reviews currently cover Azure Monitor collection architecture, dedicated Log Analytics clusters, custom Azure Policy definitions, Application Gateway rewrite behaviour, Queue service CORS, Windows Autopilot profiles, Intune detected software, Entra domains and external identity providers, entitlement connected organisations, catalogues and access packages. Generic service inventory, usage, activity feeds and detail lookups remain viewable context and do not create analyst work.
- **Export Validated SARIF** downloads `azure-findings-validated-SARIF.json` containing confirmed `found` results and confirmed manual-assessment items. Raised findings retain their original SARIF result; manual items are emitted only in this validated export as `kind: review` and `level: note`, with their evidence, provenance and current analyst review. Export metadata records total validated items, raised findings and manual assessments separately. The export is independent of the visible status and search filters. The ordinary `azure-findings-SARIF.json` remains found-only and must remain alongside the flat findings file.
...
- The dashboard has a dedicated **Microsoft Graph Collection** section derived from the latest manifest. It shows selected Graph endpoints, endpoints returning data, records written, the overall Graph outcome and status totals. The filterable Microsoft Graph endpoint-results panel in Collection Request Health provides the corresponding execution detail. Together these are the authoritative in-product indication that Graph returned and retained assessment data.
- Data Viewer keeps human-friendly dataset names and adds **Collection Source** and **Workload** columns, so Microsoft Graph data can be identified without knowing technical filenames or endpoint IDs. Graph workload and API-channel labels are shown in the dataset index. The working dataset dropdown uses the friendly dataset name alone so repeated source/workload prefixes do not obscure the list.
...
- Data sources in the Data Viewer index and selector are ordered alphabetically by their friendly display name. **Search All Data** performs a case-insensitive free-text search across keys and values in every retained version of every dataset exposed through the Data Viewer. Results identify the friendly source and version, link back to the matching dataset, and are presented 100 records per page without limiting the total files scanned. Findings and collection manifests remain in their dedicated views and are not included in this search.
- If `azure-findings-flat.json` exists in the input directory, findings are also available through the dashboard.
- The **Finding Outcome Distribution** pie counts every automated check or manual-review item once across all Azure and Microsoft Graph endpoint types. It consolidates automated results into Findings Raised, Checks Passed, Missing Source, Empty Upstream Source, Scope Restricted, Unlicensed, Failed, Unauthorised and Missing Prerequisite, and separately counts Manual Assessment Required; endpoint families do not fragment those slices.
- The adjacent relationship-weighted Sankey joins manifest endpoint executions to finding provenance by stable endpoint ID. Automated paths flow from endpoint type and endpoint outcome through finding-check group and check status to Passed, Failed, Nothing to Assess or Could Not Assess. At the finding-result stage, **Failed** means the assessment check raised a finding; it does not mean the collection request failed. **Nothing to Assess** is limited to a complete, applicable, licensed and access-verified source that returned no applicable records; collection, permission, licence, visibility and implementation gaps end at **Could Not Assess**. Supporting-only executions terminate at **Supporting Endpoint Execution — Not Assessed**, while context-only inventory terminates at **Context or Inventory — No Assessment**. Curated manual-review evidence flows through its finding-check group to **Awaiting Analyst Assessment** in the finding-check-status stage, then stops without an artificial result. Band widths count endpoint-to-check dependency relationships, while node labels retain the distinct endpoint-execution and finding-check counts. A declared check whose required execution is absent is shown under its Base or Microsoft Graph family as **Endpoint Not Recorded**. `Unattributed Endpoint` is retained only for historical finding files that pre-date endpoint-source declarations.
- Curated manual endpoints create one stable **Manual Assessment Required** item per endpoint definition, regardless of fan-out execution count, and are available through that status in the Findings viewer. Its title is the friendly endpoint name, its reason is intentionally blank, and its displayed count is the number of records in retained, viewable source datasets. Viewer links open the complete source dataset without applying the dataset name as a record filter. Failed, unauthorised, missing, non-viewable or empty manual sources remain `no_data_to_assess` and do not appear under the manual status. Supporting-only and context-only executions never create manual findings; the Sankey records them separately as supporting or context/inventory collection. Manual items remain absent from ordinary SARIF but can be analyst-confirmed and included in the validated SARIF export.
- Endpoint-to-finding flows are shown only when the findings provenance and collection manifest have the same collection-run ID. If the findings belong to an older retained run, its matching historical manifest is used and the dashboard displays a warning. If no matching manifest exists, the flow is withheld rather than combining unrelated runs; regenerate `azure-findings-flat.json` from the intended collection directory.
...
- The dashboard uses one pie chart for mutually exclusive finding outcomes and one adjacent Sankey joining the endpoint and finding populations. The collection-type text states both denominators explicitly. Request Attempt Outcomes, Skipped Endpoint Definitions and Unrecorded Endpoint Definitions combine Azure base, Azure parameterised and Microsoft Graph records. Separate filterable panels provide successful and unsuccessful requests across both sources, plus complete Base and Microsoft Graph endpoint-result inventories.
...
The runner announces the number of selected Graph endpoints and prints start and completion lines such as `Graph [12/88]`. Page progress is shown for the first page and then every tenth page, fan-out progress is shown for the first, every twenty-fifth and final parent request, and an unchanged audit-query state produces a heartbeat every fifteen polls (normally about 30 seconds). A completion line reports the endpoint outcome, streamed record count, page count and request attempts, including missing permission names where the token itself proves the gap. Single-part streamed datasets are published atomically without being parsed and written a second time. Where fan-out genuinely produces multiple parts, consolidation and required parent-ID indexing are also streamed and emit bounded progress messages. Individual Graph records may exceed the normal streaming read chunk without imposing a client-side record-size cap.
