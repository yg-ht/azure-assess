...
- The **Finding Outcome Distribution** pie counts every assessment check once across all Azure and Microsoft Graph endpoint types. It consolidates results into Findings Raised, Checks Passed, Missing Source, Empty Upstream Source, Scope Restricted, Unlicensed, Failed, Unauthorised and Missing Prerequisite; endpoint families do not fragment those slices.
- The adjacent relationship-weighted Sankey joins manifest endpoint executions to finding provenance by stable endpoint ID. It flows from endpoint type and endpoint outcome through finding-check group and check status to Passed, Failed, Nothing to Assess or Could Not Assess. At the finding-result stage, **Failed** means the assessment check raised a finding; it does not mean the collection request failed. **Nothing to Assess** is limited to a complete, applicable, licensed and access-verified source that returned no applicable records; collection, permission, licence, visibility and implementation gaps end at **Could Not Assess**. Band widths count endpoint-to-check dependency relationships, while node labels retain the distinct endpoint-execution and finding-check counts. A declared check whose required execution is absent is shown under its Base or Microsoft Graph family as **Endpoint Not Recorded**. `Unattributed Endpoint` is retained only for historical finding files that pre-date endpoint-source declarations; unlinked manifest endpoints also remain visible.
