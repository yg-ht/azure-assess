...
- The collection endpoint outcome chart counts every remote endpoint execution in the latest manifest once, across Microsoft Graph, base and parameterised collection. Its collection-type breakdown exposes which type produced unauthorised, failed, incomplete, empty, skipped and other outcomes. Local Azure CLI setup operations are excluded because they do not call a collection endpoint.
...
- The dashboard keeps finding checks and collection endpoint executions in separate pie charts because they use different denominators. Checks without sufficient data are divided by one primary cause from their required endpoint provenance: unauthorised source, unavailable tenant licence or capability, failed request, interrupted or unrecorded collection, skipped prerequisite, empty upstream source, or missing/unattributed source. The collection chart independently reconciles every remote manifest endpoint execution to one outcome.
