...
- The adjacent Sankey uses the same finding population and shows the complete path from All Endpoints through endpoint family, authorisation/request state, data availability and assessment reason to Checks Passed, Findings Raised or Insufficient Data. **Base Endpoints** includes Azure base and parameterised collection; mixed Graph/Azure and unattributed checks remain explicit so no source type is hidden. Exact manifest endpoint-execution outcomes remain available in the collection-type breakdown and request-health tables.
...
- The dashboard uses one pie chart for mutually exclusive finding outcomes and one adjacent Sankey over the same finding population. Raw endpoint executions are not added to that denominator; their manifest outcomes instead supply endpoint-type attribution and remain separately reconciled in the request-health section.
