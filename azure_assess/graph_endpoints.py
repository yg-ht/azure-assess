...
GRAPH_ENDPOINTS: List[Dict] = [
    # Identity baseline
    E("applications", "Graph Applications", "IdentityBaseline", "Application.Read.All", "/applications"),
    ...
    E("sign_ins", "Graph Sign-ins", "IdentityBaseline", "AuditLog.Read.All", "/auditLogs/signIns?$filter=createdDateTime ge {start}", api="beta"),
    ...
    E("identity_providers", "Graph Identity Providers", "IdentityBaseline", "IdentityProvider.Read.All", "/identity/identityProviders", api="beta"),
    ...
    E("identity_policies", "Graph Identity Policies", "IdentityBaseline", "Policy.Read.All", "/policies/authorizationPolicy", pagination=False),
    ...
    E("provisioning_logs", "Graph Provisioning Logs", "IdentityBaseline", "ProvisioningLog.Read.All", "/auditLogs/provisioning?$filter=activityDateTime ge {start}", api="beta"),
    ...
    E("directory_role_assignments", "Graph Directory Role Assignments", "IdentityBaseline", "RoleManagement.Read.All", "/roleManagement/directory/roleAssignments"),
    E("user_registration_details", "Graph User Registration Details", "IdentityBaseline", "UserAuthenticationMethod.Read.All", "/reports/authenticationMethods/userRegistrationDetails"),
    ...
    E("users", "Graph Users", "IdentityBaseline", "Directory.Read.All", "/users?$select=id,displayName,userPrincipalName,accountEnabled"),
    ...
    E("activity_exchange", "Exchange Service Activity", "M365Audit", "ServiceActivity-Exchange.Read.All", "/reports/serviceActivity/getEmailActivityUserDetail(period='D30')"),
    E("activity_m365_web", "Microsoft 365 Web Activity", "M365Audit", "ServiceActivity-Microsoft365Web.Read.All", "/reports/serviceActivity/getOffice365ActiveUserDetail(period='D30')"),
    E("activity_onedrive", "OneDrive Service Activity", "M365Audit", "ServiceActivity-OneDrive.Read.All", "/reports/serviceActivity/getOneDriveActivityUserDetail(period='D30')"),
    E("activity_teams", "Teams Service Activity", "M365Audit", "ServiceActivity-Teams.Read.All", "/reports/serviceActivity/getTeamsUserActivityUserDetail(period='D30')"),
    ...
    E("settings_sharepoint", "SharePoint Tenant Settings", "M365Audit", "SharePointTenantSettings.Read.All", "/admin/sharepoint/settings", api="beta", pagination=False),
...
for query in HUNTING_QUERY_PACK["queries"]:
    GRAPH_ENDPOINTS.append(E(
        ...
        body={"Query": query["query"], "queryPackVersion": HUNTING_QUERY_PACK["version"]},
    ))
...
def validate_registry(endpoints: Iterable[Mapping] = GRAPH_ENDPOINTS) -> None:
    ...
    for endpoint in endpoints:
        missing = {"id", "name", "profile", "permission", "api", "method", "path", "pagination", "output", "licence"} - set(endpoint)
        if missing:
