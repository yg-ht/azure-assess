...
INTERNET_SOURCE_TAGS = {"*", "internet", "0.0.0.0/0", "::/0"}
...
def external_source(rule: Mapping[str, Any]) -> bool:
    for value in values(rule, "sourceAddressPrefix", "sourceAddressPrefixes"):
        ...
        try:
            ...
            continue
        # A public allow-list is still externally reachable, even when it is
        # narrower than the Internet service tag or the default route.
        if network.is_global:
            return True
    return False


def rule_matches(rule: Mapping[str, Any], protocol: str, port: int) -> bool:
    if normalise_identifier(nested_value(rule, "direction", ("properties", "direction"))) != "inbound":
        return False
    ...
    if rule_protocol not in {"*", "any", normalise_identifier(protocol)}:
        return False
    if not external_source(rule):
        return False
    return any(start <= port <= end for start, end in port_intervals(rule))


def nsg_decision(rules: Iterable[Mapping[str, Any]], protocol: str, port: int) -> Dict[str, Any]:
    """Apply Azure inbound priority ordering for an Internet-origin port."""
    matching = [rule for rule in rules if rule_matches(rule, protocol, port)]
    if not matching:
        return {"decision": "unknown", "rule": None}
    matching.sort(
        key=lambda rule: (
            int(nested_value(rule, "priority", ("properties", "priority")) or 65535),
            str(rule.get("name") or ""),
        )
    )
    selected = matching[0]
    access = normalise_identifier(nested_value(selected, "access", ("properties", "access")))
    return {"decision": access if access in {"allow", "deny"} else "unknown", "rule": selected}
...
def nic_nsg_rules(nic: Mapping[str, Any], nsgs_by_id: Mapping[str, Mapping[str, Any]]):
    ...
    return list(nested_value(nsg or {}, "securityRules", ("properties", "securityRules")) or [])
...
) -> Tuple[Dict[str, Tuple[Mapping[str, Any], Mapping[str, Any]]], Dict[str, List[Mapping[str, Any]]]]:
    by_id = {}
    pool_nics: Dict[str, List[Mapping[str, Any]]] = {}
    for nic in nics:
        ...
        for configuration in configurations:
            ...
            for pool in nested_value(
                ...
                if pool_id:
                    pool_nics.setdefault(pool_id, []).append(nic)
    return by_id, pool_nics
...
def decision_for_nic(
    ...
    if rule_groups:
        group_decisions = [nsg_decision(rules, protocol, port) for rules in rule_groups]
        denied = next(
        ...
        decision = denied or unknown or group_decisions[0]
        decision["decision"] = (
            "deny" if denied else "unknown" if unknown else "allow"
        )
        decision["source"] = "effective_nsg"
        return decision

    rules = nic_nsg_rules(nic, nsgs_by_id)
    decision = nsg_decision(rules, protocol, port) if rules else {
        "decision": "unknown",
        "rule": None,
    }
    decision["source"] = "raw_nsg" if rules else "unavailable"
    return decision
...
def allowed_nic_segments(
    ...
    rules = (
        [rule for group in effective_groups for rule in group]
        if effective_groups
        else nic_nsg_rules(nic, nsgs_by_id)
    )
    protocols = set()
    for rule in rules:
        if not external_source(rule):
    ...
    for protocol in sorted(item for item in protocols if item):
        ...
        for rule in rules:
            ...
            if not external_source(rule) or rule_protocol not in {"*", "any", protocol}:
                continue
        ...
        for start, next_boundary in zip(ordered, ordered[1:]):
            ...
            decision = decision_for_nic(
                nic, protocol, start, nsgs_by_id, effective_by_nic
            )
    ...
    for nic in nics:
        ...
        for configuration in configurations:
            ...
            for protocol, start_port, end_port, decision in allowed_nic_segments(
                nic, nsgs_by_id, effective_by_nic
            ):
...
def load_balancer_paths(
    ...
    ips = public_ip_index(public_ips)
    nics = list(nics)
    ip_configurations, pool_nics = nic_ip_configuration_index(nics)
    nsgs_by_id = nsg_index(nsgs)
    effective_by_nic = effective_rules_index(effective_nsgs)
    routes_by_nic = route_index(effective_routes)
    paths = []
    for balancer in load_balancers:
        ...
        for rule in rules:
            ...
            backends = pool_nics.get(pool_id, [])
            ...
            if direct_backend:
                backends = list(backends) + [direct_backend[0]]
            backends = list(
                {
                    canonical_arm_id(item.get("id")): item
            ...
            for nic in backends:
                decision = decision_for_nic(
                    nic, protocol, backend_port, nsgs_by_id, effective_by_nic
                )
...
def analyse_external_attack_paths(
    ...
    paths = direct_nic_paths(
        public_ips, nics, nsgs, effective_nsgs, effective_routes
    )
    paths.extend(
        load_balancer_paths(
            public_ips,
